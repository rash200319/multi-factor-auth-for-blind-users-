 ...
 * gone. Redeeming a code re-activates the account but the entire set is
 * invalidated and a replacement authenticator enrolment is required next
 * (readme.md §6.5, PDF §9.2) — handled by /register/begin once status is
 * RECOVERY.
 */
recoveryRouter.post("/redeem", async (req, res) => {
  ...
  const result = await verifyAndConsumeRecoveryCode(userId, code);
  if (result !== "ok") {
    return res.status(400).json({ result });
  }

  run("UPDATE users SET status = 'RECOVERY' WHERE id = ?", [userId]);
  audit(userId, "lifecycle.recovery_pending_replacement");

  const token = await issueSession(userId, "aal2-elevated");
  res.cookie(sessionCookieName, token, sessionCookieOptions);
  res.json({ result: "ok", accountStatus: "RECOVERY", nextStep: "register_replacement_authenticator" });
});
